...
export async function showProfileContent(root: HTMLElement): Promise<HTMLElement> {
  ...
  let customer;
  await getCustomerByEmail(emailCur).then((body) => (customer = body.body.results[0]));
...
async function saveUserInfo(e: Event) {
  e.preventDefault();
  ...
  if (input.id === UserFNamelInput.id)
    await updateUserName(data.id, input.value, Number(data.version)).then(({ body }) => (data = body));

  if (input.id === UserLNamelInput.id)
    await updateUserLName(data.id, input.value, Number(data.version)).then(({ body }) => (data = body));

  if (input.id === UserLBirthlInput.id)
    await updateUserBDay(data.id, input.value, Number(data.version)).then(({ body }) => (data = body));
  if (input.id === LoginEmailInput.id)
    await updateUserEmail(data.id, input.value, Number(data.version)).then(({ body }) => (data = body));
  alert('succes');
